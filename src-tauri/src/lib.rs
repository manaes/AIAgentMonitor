...
///
/// **LAN 은 같은 이유가 아니다.** 윗 문단의 위험(인가가 살아 있을 때 봉인해 둔
/// 프레임이 해제 뒤에 나간다)이 LAN 에는 없다 — 해제는 `revoke_peer` 가 먼저
/// 인가를 지우고 나서 여기로 오므로 그 창 자체가 열리지 않는다. 자리 회수도
/// 이 배치와 인과가 없다: 저장 뒤로 밀려도 `drop_sessions` 는 디스크 쓰기 바로
/// 다음에 도니 자리는 밀리초 늦게 돌아올 뿐이다.
///
