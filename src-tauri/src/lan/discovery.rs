...
//! 없다.** 패널이 LAN 주소를 **리스너가 서 있는 동안 언제나** 함께 보여주는 것이
//! 그 대비다(`local_ipv4`, 배선은 Task 6). 기준이 토글이 아니라 리스너인 것은
//! bind 에 실패한 포트를 손으로 넣게 하지 않기 위해서다(`LanBridge::is_listening`) —
//! 게시가 죽은 포트를 광고하지 않는 것과 같은 규칙이고, 게시 실패는 여기서 보듯
//! 감지되지 않으므로 **그것을 근거로 주소를 감추지는 않는다.** 이 사실을 doc 에
//! 적어 두는 이유는, 크레이트의 `new()` doc 이
