...
// AI Agent Monitor — CYD 펌웨어: LVGL 설정 파일 (Task 14a, 디스플레이·터치 브링업)
//
// `esp32_smartdisplay` 의 `library.json` 은 `"dependencies": {"lvgl/lvgl":
// "^9.2.2"}` 로 v9.2.2 이상을 요구한다고만 적었지만, `^` 범위라 `pio run`
// 이 실제로 받아 온 것은 **v9.5.0**(2026-08-27 실측, `pio run` 로그의
// `Library Manager: lvgl@9.5.0 has been installed!`) — Task 13 이 폰트
// 폴백 동작을 확인할 때 이미 인용한 바로 그 버전(`tools/build-font.sh`
// 상단 T13-B 절, `lv_font.c` 인용)과 같다. 그래서 이 파일은 처음엔 착각해서
// git 태그 `v9.2.2` 의 `lv_conf_template.h` 로 만들었다가, 실제로 받은
// 패키지(`.pio/libdeps/cyd/lvgl/lv_conf_template.h`, 1526줄 — v9.2.2 템플릿
// 은 1127줄이라 새 옵션이 400줄 가까이 늘었다)로 다시 만들었다 — **실제로
// 받은 버전과 다른 버전 문서를 베낀 채로 두면 다음 태스크가 없는 옵션을
// 찾아 헤맬 수 있어서**다.
//
// 그 실제 템플릿을 그대로 복사해 아래 네 곳만 고쳤다. 그 외 값은 LVGL
// 기본값이다 — 손대지 않았다는 뜻이다.
//
//   1. `#if 0` → `#if 1` — 이걸 안 하면 파일 전체가 주석 취급돼 아무 설정도
//      적용되지 않는다(템플릿 자체의 경고). **줄 번호를 여기 적지 않는다**
//      — 이 설명 블록 자체가 파일 맨 위에 있어서 블록 길이가 바뀔 때마다
//      아래 값들의 실제 줄 번호가 밀린다(fix round 1 에서 실제로 겪은
//      실수 — `grep -n "^#if 1" include/lv_conf.h` 로 그때그때 확인해라).
//   2. `LV_USE_LOG` 0 → 1, `LV_LOG_PRINTF` 0 → 1 — 화면 없는 기기의 유일한
//      디버그 통로가 시리얼이라는 게 이 브랜치 전체의 전제(`main.cpp` T9-D
//      직전 주석)라서, README 가 권장한 대로 켰다. `LV_LOG_PRINTF 1` 은
//      `lv_log_register_print_cb()` 없이 그냥 `printf` 로 찍으라는 뜻이고,
//      이 프로젝트의 arduino-esp32 코어는 `printf` 를 이미 `Serial`(UART0)로
//      돌린다 — 별도 콜백을 등록할 이유가 없다.
//   3. **`LV_USE_FONT_COMPRESSED` 0 → 1 — 실기에서 실측해서 찾은 값이다.**
//      `font_ko`(Task 13, `tools/build-font.sh` 가 `lv_font_conv` 를
//      `--no-compress` 없이 돌려서 기본값인 RLE 압축 글리프로 구웠다)를
//      이 태스크가 처음으로 실제 화면에 그려 봤더니, 이 값이 기본값(0)일 때
//      시리얼에 `lv_font_get_bitmap_fmt_txt: Compressed fonts is used but
//      LV_USE_FONT_COMPRESSED is not enabled` 경고가 글자마다 찍히고 그
//      글자는 아예 안 그려졌다(`draw_letter_cb: Couldn't get the bitmap of
//      a glyph`) — `LV_USE_FONT_PLACEHOLDER=0` 일 때의 "폭 0으로 사라짐"과는
//      다른, 하지만 마찬가지로 "읽을 수 없다"인 실패 모드다. 1로 켜니
//      경고가 완전히 사라졌다(태스크 보고서 참고). Task 13 은 이 폰트를
//      굽기만 하고 실기에 그려 본 적이 없어서 이 문제가 이 태스크 전까지
//      드러나지 않았다.
//   4. **바꾸지 않은 것, 그래서 확인해 둘 값 둘(여기도 줄 번호는 안 적는다
//      — 3번 항목과 같은 이유. `grep -n` 으로 확인해라. Task 13 이 인용한
//      `708번째 줄`은 순정 템플릿 기준이라 이 파일에는 안 맞는다 — 우리
//      파일 맨 위의 이 설명 블록만큼 밀려 있다):**
//      - `LV_COLOR_DEPTH 16` — 템플릿 기본값이 이미 이 패널이 지원하는
//        유일한 형식(RGB565)과 같다. `esp32-smartdisplay` README
//        "Important settings" 절이 이 값을 명시적으로 요구한다.
//      - `LV_USE_FONT_PLACEHOLDER 1` — 템플릿 기본값이 이미 1 이고, 그대로
//        뒀다. Task 13 이 확인해 둔 제약: 이 값이 0 이면 `font_ko` 서브셋
//        밖 글자(프로젝트 이름은 임의의 UTF-8)가 폭 0 으로 조용히 사라진다.
//        **이 파일에서 처음으로 실제로 켜지는 자리이므로 여기서 재확인한다
//        — grep 결과는 태스크 보고서에 남긴다.**
//
// `platformio.ini` 의 `build_flags` 에 절대경로로 넘기는
// `-D LV_CONF_PATH="${platformio.include_dir}/lv_conf.h"` 가 이 파일을
// 가리켜야 LVGL 이 이 설정을 쓴다. **캐싱 함정을 실제로 겪었다**(T14a-C,
// 태스크 보고서 참고): `LV_USE_FONT_COMPRESSED` 를 고친 뒤 `.pio/` 를
// 지우지 않고 다시 빌드했더니 빌드는 성공했지만 산출물 크기가 바이트
// 단위로 완전히 같았고(재컴파일 안 됨) 실기 시리얼 로그도 그대로였다 —
// `rm -rf .pio` 뒤 다시 빌드하고 나서야 반영됐다.
