...
// 기본값이다 — 손대지 않았다는 뜻이다.
//
//   1. 15번째 줄 `#if 0` → `#if 1` — 이걸 안 하면 파일 전체가 주석 취급돼
//      아무 설정도 적용되지 않는다(템플릿 자체의 경고).
...
//      드러나지 않았다.
//   4. **바꾸지 않은 것, 그래서 확인해 둘 값 둘:**
//      - `LV_COLOR_DEPTH 16`(30번째 줄) — 템플릿 기본값이 이미 이 패널이
//        지원하는 유일한 형식(RGB565)과 같다. `esp32-smartdisplay` README
//        "Important settings" 절이 이 값을 명시적으로 요구한다.
//      - `LV_USE_FONT_PLACEHOLDER 1`(708번째 줄, v9.5.0 템플릿 기준 — Task 13
//        이 인용한 줄 번호와 일치) — 템플릿 기본값이 이미 1 이고, 그대로
