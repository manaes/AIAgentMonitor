...
impl TrayIcon {
    ...
    fn create(
        ...
        // PATCH(macOS 27, AI Agent Monitor): 여기서 setMenu 를 하지 않는다.
        // NSStatusItem 에 메뉴가 붙어 있으면 macOS 27 부터 좌클릭이 TrayTarget
        // 서브뷰까지 전달되지 않고 시스템이 메뉴를 먼저 띄워 버린다
        // (tauri-apps/tray-icon#355). 메뉴는 on_tray_click 이 클릭을 받은
        // **뒤에** 잠깐 붙였다 뗀다 — upstream PR #341 과 같은 접근.
        // 자세한 내용은 vendor/README.md 참고.
    ...
    pub fn set_menu(&mut self, menu: Option<Box<dyn menu::ContextMenu>>) {
        if let (Some(ns_status_item), Some(tray_target)) = (&self.ns_status_item, &self.tray_target)
        {
            unsafe {
                ...
                // PATCH: 붙이는 건 클릭 시점으로 미룬다(위 create() 주석 참고).
                // delegate 는 메뉴 열림/닫힘 때 NSStatusItem 이 하이라이트를
                // 되돌리는 데 쓰이므로 그대로 걸어 둔다.
    ...
    pub fn show_menu(&self) {
        if let (Some(ns_status_item), Some(tray_target)) = (&self.ns_status_item, &self.tray_target)
        {
            // PATCH: 메뉴가 상시로는 안 붙어 있으므로 여기서도 붙였다 뗀다.
            let menu = tray_target.ivars().menu.borrow().clone();
            unsafe {
                let button = ns_status_item.button(self.mtm).unwrap();
                match menu {
                    Some(menu) => pop_up_menu(ns_status_item, &button, &menu),
                    None => button.performClick(None),
                }
            }
        }
    }
...
fn on_tray_click(this: &TrayTarget, button: MouseButton) {
    ...
    unsafe {
        ...
        {
            // PATCH: RefCell 빌림을 performClick 의 **중첩 런루프** 너머까지
            // 들고 있으면, 메뉴가 떠 있는 동안 set_menu 가 불릴 때
            // BorrowMutError 로 죽는다(upstream #365). Retained 를 먼저 복제해
            // 꺼내고 빌림을 즉시 끝낸다.
            let menu = this.ivars().menu.borrow().clone();
            match menu.filter(|m| m.numberOfItems() > 0) {
                Some(menu) => pop_up_menu(&this.ivars().status_item, &ns_button, &menu),
                None => ns_button.highlight(true),
            }
        } else {
...
/// PATCH(macOS 27): 메뉴를 **클릭을 받은 직후에만** 붙였다가 곧바로 떼어낸다.
///
/// 상시로 붙여 두면 macOS 27 이 좌클릭을 서브뷰(TrayTarget)까지 내려보내지 않아
/// `TrayIconEvent::Click` 이 아예 발생하지 않는다(tauri-apps/tray-icon#355).
/// `performClick` 은 메뉴가 닫힐 때까지 돌아오지 않는 **중첩 런루프**라, 이 함수가
/// 반환된 시점에는 메뉴가 이미 닫혀 있어 곧바로 떼어내도 안전하다.
