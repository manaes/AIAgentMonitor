...
//! 그래서 인증보다 **앞에** 있는 자원에 상한을 걸었다: 프레임 크기, 동시 연결 수,
//! 이벤트 큐, central 별 송신 큐, 그리고 **시간**. 상한을 넘긴 쪽은 기다리게
//! 하지 않고 **놓는다** — 여기서 기다리면 낯선 기기 하나가 서버 전체를 세울 수
//! 있다.
//!
//! ### 그 상한이 서는 계층 — 전부 WebSocket 쪽이다
//! 프레임 크기(`MAX_FRAME_BYTES`)·이벤트 큐(`EVENT_QUEUE`)·송신 큐(`SINK_QUEUE`)는
//! 업그레이드된 연결에 붙는다. 동시 연결 수(`MAX_CONNECTIONS`)는 `upgrade()` 안에서,
//! 시간(`AUTH_DEADLINE`)은 `handle()` 안에서 잡힌다. 그리고 `upgrade()` 는
//! **axum 이 완결된 HTTP 요청을 파싱한 뒤에야** 돈다.
//!
//! ### 업그레이드 이전의 TCP 연결에는 개수 상한도 시간 상한도 없다
//! 접속만 하고 한 바이트도 보내지 않는 소켓은 자리표를 쓰지 않는다. 그리고
//! `axum::serve(listener, app)` 에는 헤더 읽기 타임아웃도 동시 연결 제한도 붙어
//! 있지 않다 — `tower` 는 의존성에 있지만 이 라우터에 레이어를 얹지 않았다.
//! 실측된 모양이다: `MAX_CONNECTIONS` 의 여덟 배인 64개 소켓을 열어 두어도
//! 정상 업그레이드가 **여전히 101 을 받고**, 5초 뒤에도 그 64개가 **전부 살아
//! 있다.** 즉 위 목록의 다섯 중 "동시 연결 수"와 "시간"은 **TCP 계층에서는
//! 성립하지 않는다.**
//!
//! 실패 모드는 FD 고갈이고, **그때 죽는 것은 LAN 만이 아니다** — quota
//! 프록시(4319), 디스크 쓰기, `save_paired_peers` 가 전부 같은 프로세스의 FD
//! 예산을 쓴다. 세 보안 성질(미인증 0바이트 · `(키, 논스)` 재사용 없음 · 평문
//! 미유출)은 어느 것도 깨지지 않으므로 가용성만의 문제이고, LAN 공유는 기본
//! 꺼짐이다. **하지만 닫혀 있지는 않다.** 닫으려면 accept 계층 자체에 상한을
//! 걸어야 한다 — `axum::serve` 를 `listener.accept()` 루프로 바꾸고 거기서 개수와
//! 헤더 읽기 시간을 재는 일이며, **아직 하지 않았다.** 이 절을 "인증 앞의 자원은
//! 전부 상한이 있다"로 읽으면 안 된다.
//!
...
//! 실질적으로 닫혀 있다.
//!
//! **그리고 자리를 아예 잡지 않는 구간이 그 앞에 하나 더 있다** — 업그레이드
//! 이전의 TCP 연결이다. 셋 중 어느 시한도 거기에는 닿지 않는다. 위의 「업그레이드
//! 이전의 TCP 연결에는 개수 상한도 시간 상한도 없다」 절이 그 얘기다.
//!
...
//!
//! ## 알려진 한계 — 이벤트 큐는 여덟 연결이 공유한다
//! `EVENT_QUEUE`(256)는 연결마다가 아니라 서버에 하나뿐이다. `offer_frame` 의
//! `try_send` 가 실패하면 **그 프레임을 보낸 연결**을 놓는다(`handle()` 의 수신
//! 가지). 쏟아붓는 피어는 그렇게 스스로 퇴장한다. 그런데 **큐가 차 있는 동안
//! 도착한 얌전한 CYD 의 프레임도 같은 규칙에 걸린다** — 큐를 채운 책임이 없는
//! 기기가 놓이고, 그 기기는 끊겼다 붙기를 반복한다.
//!
//! **그래도 받아들인 한계다.** 메모리 상한은 유지되고(큐는 256 을 넘지 않는다)
//! 세 보안 성질 중 어느 것도 건드리지 않는다 — 가용성만의 문제다. 제대로 닫으려면
//! 연결별 프레임 예산이 필요하고 그것은 수신 경로의 재설계이므로, 이 라운드에서
//! 하지 않았다. **하지 않았다는 것을 아는 채로 남겨 둔다.**
...
///
/// 살아 있는 기기는 Pong 을 되돌려주고(WebSocket 규약), 그 Pong 도 "무언가
/// 받았다"로 친다 — 화면이 멈춰 있어도 유지된다는 뜻이다.
///
/// **다만 그 Pong 을 보내는 것은 OS 의 TCP 스택이 아니라 클라이언트의
/// WebSocket 라이브러리다.** 계획서가 고른 `links2004/WebSockets`
/// (`docs/superpowers/plans/2026-08-25-cyd-client.md` Task 8)는
/// `webSocket.loop()` 가 불릴 때만 Pong 을 돌려준다. 즉 펌웨어의 메인 루프가
/// `IDLE_TIMEOUT`(90초)보다 오래 블로킹하면(긴 `delay()`, 느린 전체 화면 갱신,
/// 블로킹 WiFi 재접속) WiFi 도 소켓도 멀쩡한 보드가 **여기서 끊긴다** — 그리고
/// 그 기기에는 이유를 설명할 화면도 키보드도 없다. 그래서 펌웨어 쪽 제약(메인
/// 루프의 블로킹 예산)은 계획서 Task 8 에 명시해 두었다. 이 90초를 바꾸면 그
/// 예산도 함께 움직인다.
...
impl Slots {
    ...
    /// 지금 몇 자리가 나가 있는가. 자리 회수(`Slot::drop`)를 확인할 때만 쓴다.
    #[cfg(test)]
    fn live(&self) -> usize {
...
async fn handle(
    ...
    // 멈춰 있어도 살아 있는 것으로 본다. 다만 그 Pong 은 OS 가 아니라 상대의
    // WebSocket 라이브러리가 보내므로, 상대의 메인 루프가 멈추면 이 값도 멈춘다
    // (`PING_INTERVAL` 의 doc).
