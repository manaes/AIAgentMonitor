...
public enum CryptoV2 {
    /// HKDF info 문자열. 두 언어가 바이트 단위로 같아야 한다.
    public static let infoPair = Data("aim-pair-v2".utf8)
    public static let infoS2C = Data("aim-sess-v2-s2c".utf8)
    public static let infoC2S = Data("aim-sess-v2-c2s".utf8)
    /// AEAD 부가 인증 데이터. 프로토콜 버전을 태그에 묶는다.
    public static let aad = Data("aim-v2".utf8)

    /// 두 임시 공개키를 이어붙인 64바이트. **항상 클라이언트 키가 먼저다** —
    /// 역할과 무관하게 양쪽이 같은 순서로 만들어야 cbind 와 proof 가 일치한다.
    public static func transcript(clientPub: Data, serverPub: Data) -> Data {
        clientPub + serverPub
    }
    ...
    public static func derivePairKey(sharedSecret: Data, nonce: Data) -> SymmetricKey {
    ...
    /// 끼워넣으면 값이 맞지 않는다.
    public static func codeBinding(code: String, transcript: Data) -> Data {
    ...
    public static func sessionProof(token: Data, nonce: Data, transcript: Data) -> Data {
        Data(HMAC<SHA256>.authenticationCode(
...
public enum SealedChannelError: Error, Equatable {
    /// 카운터와 태그를 담기에도 짧다.
    case tooShort
...
/// 만져야 한다.
public final class SealedChannel {
