...
> 토큰은 영구 자격증명이고, 발급 시 1회 + **재연결마다 매번** 평문 링크를 왕복한다. 따라서 스니핑
> 1회로 지속적 접근권이 넘어간다 — 얻는 것이 스니퍼가 어차피 볼 수 있는 것과 같은 데이터라 피해
> 증분은 "나중에 범위 밖에서도 다시 붙을 수 있음"이지만, 근거 문장 자체는 더 이상 사실이 아니다.
> 토큰이 링크를 타지 않게 하려면 재인증을 챌린지-응답(Mac이 논스를 notify, 클라이언트가 HMAC 응답)
> 으로 바꿔야 하며, Rust·Swift 양쪽에 암호 의존성이 추가된다. **현재 범위 밖이며 사용자 판단 사안이다.**
